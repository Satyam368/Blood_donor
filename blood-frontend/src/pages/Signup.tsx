...
const zipRegex = /^[0-9A-Za-z -]{3,10}$/;

const signupSchema = z.object({
  firstName: z.string().optional(),
  lastName: z.string().optional(),
  email: z.string().min(1, "Email is required").email("Invalid email format"),
  phone: z.string().min(1, "Phone is required").regex(phoneRegex, "Phone number must be 8-16 digits and may include spaces or dashes"),
  password: z.string().min(1, "Password is required").regex(passwordRegex, "Password must include upper, lower, number and special character"),
  confirmPassword: z.string().min(1, "Confirm your password"),
  bloodType: z.string().optional(),
  dateOfBirth: z.string().optional(),
  address: z.string().optional(),
  city: z.string().optional(),
  zipCode: z.string().optional().refine(v => !v || zipRegex.test(v), "ZIP/Postal code must be 3-10 characters (letters, numbers, space or -)"),
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (errorData && errorData.fields) {
        Object.entries<string>(errorData.fields).forEach(([field, message]) => {
          // @ts-ignore map backend field names to form fields
          form.setError(field as keyof SignupForm, { type: "server", message });
        });
      }
      alert("❌ Signup failed. Please correct the highlighted fields.");
    }
  } catch (error) {
